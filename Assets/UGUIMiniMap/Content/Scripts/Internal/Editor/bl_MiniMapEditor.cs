...
public class bl_MiniMapEditor : Editor
{
    ...
    private void OnEnable()
    {
        ...
        zoomProp = serializedObject.FindProperty("IncreaseHeightKey");
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (EditorGUILayout.BeginFadeGroup(GeneralAnim.faded))
        {
            ...
            if (script.renderType == bl_MiniMap.RenderType.Picture)
            {
                ...
                if (script.WorldSpace != null)
                {
                    ...
                    if (GUILayout.Button("Take ScreenShot", EditorStyles.toolbarButton, GUILayout.Width(150)))
                    {
                        SetupScreenShot();
                    }
                    GUILayout.Space(5);
        ...
        if (EditorGUILayout.BeginFadeGroup(ZoomAnim.faded))
        {
            ...
            script.IncreaseHeightKey = (KeyCode)EditorGUILayout.EnumPopup("ZoomIn Key", script.IncreaseHeightKey);
            script.DecreaseHeightKey = (KeyCode)EditorGUILayout.EnumPopup("ZoomOut Key", script.DecreaseHeightKey);
            script.DefaultHeight = EditorGUILayout.Slider("Default Zoom", script.DefaultHeight, script.MinZoom, script.MaxZoom);
            script.saveZoomInRuntime = EditorGUILayout.ToggleLeft("Remember Zoom Modifications?", script.saveZoomInRuntime, EditorStyles.toolbarButton);
        ...
        if (positionProp.isExpanded)
        {
            ...
            {
                if (GUILayout.Button("Stop Fullscreen Preview", EditorStyles.toolbarButton))
                {
            ...
            {
                if (GUILayout.Button("Preview Fullscreen", EditorStyles.toolbarButton))
                {
